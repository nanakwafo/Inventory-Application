...
<main>
    ...
    <table border="0" cellspacing="0" cellpadding="0">
        ...
        @foreach($data as $s)
            <?php $subtotal=$subtotal + $s->amount?>
            <tr>
                <td class="no">{{$count++}}</td>
                <td class="desc"><h3>{{\App\Productcode::find($s->productid)->name}}</h3></td>
                <td class="unit">GHC &nbsp;{{$s->rate}}</td>
                <td class="qty">{{$s->quantity}}</td>
                <td class="total">GHC &nbsp;{{ $s->amount}}</td>
            </tr>
        @endforeach
        </tbody>
        <tfoot>
        ...
        <tr>
            ...
            <td>GHC &nbsp;{{$purchaseorder->discount}}</td>
        </tr>
        <tr>
            <td colspan="2"></td>
